package com.inso.watch.update_lib.update;
...
class CheckUpdateTask extends AsyncTask<Void, Void, String> {
    ...
    private void parseJson(String result) {
        try {
            ...
            String  res = obj.getString(Constants.APK_RESULT);
            JSONObject res0 = new JSONObject(AesEncryptionUtil.decrypt(res));
            Log.d("parseJson:",AesEncryptionUtil.decrypt(res) );
            String updateMessage = res0.getString(Constants.APK_UPDATE_CONTENT);
            String apkUrl = res0.getString(Constants.APK_DOWNLOAD_URL);
            int apkCode = res0.getInt(Constants.APK_VERSION_CODE);
            int versionCode = AppUtils.getVersionCode(mContext);
            Log.d("parseJson:","apkurl:"+apkUrl+",updateMsg:" +updateMessage + ",apkCode:" + apkCode +",versionCode:"+versionCode);
            ...
            Toast.makeText(mContext, mContext.getString(R.string.checkerror), Toast.LENGTH_SHORT).show();
        }
    }
